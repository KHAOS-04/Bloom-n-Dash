...
public class Level1CState extends GameState {
	...
	public void init() {
		...
		// backgrounds
		temple = new Background("/Backgrounds/CherryTown.gif", 0.5, 0);
		
		// tilemap
		tileMap = new TileMap(30);
		tileMap.loadTiles("/Tilesets/PinkTiles.gif");
		tileMap.loadMap("/Maps/level1cCopy.map");
		tileMap.setPosition(140, -1);
		tileMap.setTween(1);
...
//		brp.setPosition(162, 112);
                ...
                // portal
                portal = new Portal(tileMap);
                portal.setPosition(240, 154); // new centered position
                ...
                blp = new BottomLeftPiece(tileMap);
                brp = new BottomRightPiece(tileMap);
                tlp.setPosition(232, 102);
                trp.setPosition(242, 102);
                blp.setPosition(232, 112);
                brp.setPosition(242, 112);
		...
		JukeBox.load("/SFX/BossHit.mp3", "BossHit");
		...
		JukeBox.load("/Music/fanfare.mp3", "fanfare");
                JukeBox.load("/Music/Monster.mp3", "MonsterSpawn");
	...
	public void draw(Graphics2D g) {
		...
		if(flash) {
			...
			g.fillRect(0, 0, GamePanel.WIDTH, GamePanel.HEIGHT);
                ...
                //GameOver
                if (player.getLives() == 0 && GameOver != null) {
                    int x = (GamePanel.WIDTH - GameOver.getWidth()) / 2;
                    int y = (GamePanel.HEIGHT - GameOver.getHeight()) / 2;
...
///////////////////////////////////////////////////////
	...
	private void eventDead() {
            ...
            if (eventCount == 1) {
                ...
                if (player.getLives() == 0) {
                    ...
                    try {
                        GameOver = ImageIO.read(getClass().getResourceAsStream("/HUD/GameOver.gif"));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            if (player.getLives() == 0) {
            // Display GameOver for a few seconds before transitioning to the menu
                if (eventCount == 100) {
                    tb.clear();
	...
	private void eventPortal() {
		...
		if(eventCount > 60 && eventCount < 180) {
                    energyParticles.add(
                    new EnergyParticle(tileMap, 237, 107, (int) (Math.random() * 4)));
		...
		if(eventCount == 360) {
			flash = true;
			spirit.setPosition(240, 160);
			DarkEnergy de;
			for(int i = 0; i < 20; i++) {
				de = new DarkEnergy(tileMap);
				de.setPosition(240, 160); // ← new centered position (same as spirit)
