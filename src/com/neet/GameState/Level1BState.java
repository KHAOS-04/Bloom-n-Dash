...
public class Level1BState extends GameState {
	...
	private HUD hud;
	private BufferedImage BloomNdash;
	private Title title;
	...
	public void init() {
		...
		// backgrounds
		temple = new Background("/Backgrounds/SalaminLvl.gif", 0.5, 0);
		...
		// title and subtitle
		try {
			BloomNdash = ImageIO.read(
				getClass().getResourceAsStream("/HUD/BloomNdash.gif")
			);
			
                        title = new Title(BloomNdash.getSubimage(0, 0, 146, 20));
                        title.sety(60);
                        subtitle = new Title(BloomNdash.getSubimage(0, 33, 63, 13));
                        subtitle.sety(85);
		}
		catch(Exception e) {
			e.printStackTrace();
		...
		JukeBox.load("/SFX/enemyhit.mp3", "enemyhit");
                JukeBox.load("/SFX/TamaNa.mp3", "TamaNa");
                
                // Stop previous level's music
                    JukeBox.stop("level1");

                // Load new background music for Level 2
                JukeBox.load("/Music/level-Salamin.mp3", "level-Salamin");
                

                // Play and loop new background music
                JukeBox.loop("level-Salamin", 600, JukeBox.getFrames("level-Salamin") - 2200);
...
///////////////////////////////////////////////////////
	...
	private void eventQuake() {
		...
		if(eventCount == 150) tileMap.setShaking(true, 2);
		if(eventCount == 180) {
                    JukeBox.play("TamaNa");
