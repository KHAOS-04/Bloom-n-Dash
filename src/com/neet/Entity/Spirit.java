	...
	public Spirit(TileMap tm, Player p, ArrayList<Enemy> enemies, ArrayList<Explosion> explosions) {
		...
		try {
			BufferedImage spritesheet = ImageIO.read(
					getClass().getResourceAsStream("/Sprites/Enemies/CherryMonster.gif")
			);
...
//		animation.setDelay(1);              //old version - glitchy fast animation
                animation.setDelay(4);              //new version - slower animation to each frame better
...
//			if(stepCount % 60 == 0)     // old version - slower attacks
                        if(stepCount % (20 + health/4) == 0){    // new version - faster attacks
				DarkEnergy de = new DarkEnergy(tileMap);
