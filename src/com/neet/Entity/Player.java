...
public class Player extends MapObject {
	...
	public Player(TileMap tm) {
		...
		JukeBox.load("/SFX/playercharge.mp3", "playercharge");
                JukeBox.load("/SFX/AnoYun.mp3", "AnoYun");
//                JukeBox.load("/SFX/TamaNa.mp3", "TamaNa");
	...
	public void setCharging() {
		if(knockback) return;
		if(!attacking && !upattacking && !charging && !alreadyCharge) {
                        alreadyCharge = true;
			charging = true;
...
//			dx = 0;
//		}
                // experiment moving while attacking
                if (charging) {
                    dx = 0;
                }
		...
		if(!falling) {
                    //alreadyDoubleJump = false; //moved to (jumping && !falling) condition
                    alreadyCharge = false;
                }
		
		// falling
		if(falling) {
                    if(charging) dy -= fallSpeed*0.1;
                    dy += fallSpeed;
		...
		if(teleporting) {
			...
			if(currentAction != ATTACKING) {
				...
				ar.y = (int)y - 6;
                                ...
                                //small movement on attack
                                if(upattacking) dy = 8;
                                else if(facingRight) dx = 8;
                                else dx = -8;
